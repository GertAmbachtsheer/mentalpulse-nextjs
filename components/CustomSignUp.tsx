...
export default function CustomSignUp({ onToggle }: { onToggle?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sendErr) {
        setError(
          clerkErrorFirstMessage(sendErr) ??
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (vErr) {
        setError(
          clerkErrorFirstMessage(vErr) ?? "Invalid code. Please try again."
        );
        return;
      }
      ...
      if (signUp.status === "complete") {
        ...
        if (finErr) {
          setError(
            clerkErrorFirstMessage(finErr) ?? "Could not complete sign-up."
          );
        }
      } else {
        ...
        <div className="relative flex h-dvh min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto bg-white dark:bg-background-dark shadow-xl">
          ...
          <div className="px-6 pt-6 pb-2 mt-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-base font-medium leading-normal text-center">
              Enter the verification code sent to your email. A verified email
              is required so Clerk can reach you for security checks (including
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (rErr) {
                    setError(
                      clerkErrorFirstMessage(rErr) ?? "Could not resend code."
                    );
                  }
                } finally {
                  setIsLoading(false);
    ...
    >
      ...
      <div className="relative flex h-dvh min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto bg-white dark:bg-background-dark shadow-xl">
        <div className="flex items-center p-4 pb-2 justify-between sticky top-0 bg-white/90 dark:bg-background-dark/90 backdrop-blur-sm z-10">
          <h2 className="text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center pr-12 text-slate-900 dark:text-slate-100">
            Sign Up
          </h2>
        </div>

        <div className="flex-1 overflow-y-auto">
          <h1 className="text-3xl font-bold leading-tight px-6 text-left pb-3 pt-6 text-slate-900 dark:text-slate-100">
            Create an account
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-base font-normal leading-normal pb-6 px-6">
          ...
          >
            ...
            <label className="flex items-start gap-3 py-2 cursor-pointer group">
              <div className="relative flex items-center">
                <input
                  required
                  className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 checked:bg-[#2b6cee] checked:border-[#2b6cee] transition-all focus:ring-2 focus:ring-[#2b6cee]/20 focus:outline-none"
                  type="checkbox"
                />
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400 pt-0.5 select-none">
                I agree to the{" "}
                <a
            ...
            <button
              disabled={isLoading || fetchStatus === "fetching"}
              type="submit"
              className="mt-4 flex w-full items-center justify-center rounded-full bg-[#2b6cee] py-4 px-6 text-base font-bold text-white shadow-lg shadow-[#2b6cee]/30 transition-all hover:bg-[#2b6cee]/90 hover:shadow-[#2b6cee]/40 active:scale-[0.98] disabled:opacity-50"
            >
              {isLoading || fetchStatus === "fetching"
                ? "Creating account..."
                : "Create Account"}
          ...
          <p className="text-base text-slate-600 dark:text-slate-400">
            ...
            {onToggle ? (
              <button
                type="button"
                onClick={onToggle}
                className="font-bold text-[#2b6cee] hover:text-[#2b6cee]/80 transition-colors ml-1"
              >
                Log in
              </button>
            ) : (
